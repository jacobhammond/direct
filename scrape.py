...
def main():
    ...
    update = {'i_start': i_start, 'i_end': i_end, 'i_legs': i_legs, 'i_departure': i_departure, 'title': 'JSON Response'}

    # return JSON update
    return update

class StartSignalHandler(http.server.SimpleHTTPRequestHandler):
    def do_POST(self):
        # read the header of the POST request
...
with socketserver.TCPServer(('', 8000), StartSignalHandler) as httpd:
